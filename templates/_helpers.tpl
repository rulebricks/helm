...
{{- /* Concrete object roots used to build app-side, day-scoped archive globs. */ -}}
...
{{- /* The app substitutes validated UTC {year}/{month}/{day} values before
     issuing a trace fallback query. S3/GCS need a full URL override; Azure's
     named collection keeps account/container settings and only overrides
     blob_path. Hour remains a wildcard inside the bounded day. */ -}}
{{- define "rulebricks.storage.decisionLogDayPathTemplate" -}}
{{- $storage := .Values.global.storage | default dict -}}
{{- $provider := $storage.provider | default "s3" -}}
{{- $path := include "rulebricks.storage.path" (list . "decisionLogs") -}}
{{- if eq $provider "azure-blob" }}
{{- printf "%s/year={year}/month={month}/day={day}/hour=*/*.{gz,zst}" $path -}}
{{- else if eq $provider "gcs" }}
{{- printf "%s/year={year}/month={month}/day={day}/hour=*/*.{gz,zst}" (include "rulebricks.storage.gcsBaseUrl" .) -}}
{{- else }}
{{- printf "%s/year={year}/month={month}/day={day}/hour=*/*.{gz,zst}" (include "rulebricks.storage.s3BaseUrl" .) -}}
{{- end }}
{{- end -}}
